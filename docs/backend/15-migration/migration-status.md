...
| Manufacturing — BOM (as an entity) | `FRAPPE_REFERENCE`, domain investigated 2026-09-19 | No frontend beyond the pre-existing read-only Work Order create lookup (`bomLookup.ts`) — the Manufacturing Masters (BOM) package concluded Gate B (no usable BOM frontend to canonicalize) and built nothing. Full schema/lifecycle/costing/multi-level/Operation-Routing-Workstation-classification investigation captured in `docs/backend/05-manufacturing/bom.md` regardless. Its own future scoped package per the Current Mission priority lock. |
