...
| `FIN-1E` — Chart of Accounts maintenance (create/edit/disable/delete) | Finance (frontend) | Extends the accepted read-only `FIN-1` Chart of Accounts into full maintenance: Account create (contextual child-account creation from the tree), edit (renames via ERPNext's whitelisted `update_account_number`, not a plain PUT), enable/disable, dependency-checked delete (GL Entries, children, Bank Account links, Company defaults — all server-side, re-checked at write time). Root accounts get no edit/disable/delete affordance anywhere. Bank Account (`FIN-1`) untouched. | `CLAUDE_HANDOFF` | Independent code-reviewer (fresh subagent) first pass — `CHANGES REQUIRED`: one real bug, optional-field clearing silently no-opped on Edit (`\|\| undefined` dropped by `JSON.stringify`, ERPNext PUT partial-merges) while still showing "Saved," also broke the implied Ledger→Group conversion; fixed narrowly (`387590a`), live-verified before/after on the real tenant; independent targeted re-review (fresh subagent) — `PASS`, no remaining findings; independent qa-tester (fresh subagent, live against `3c50478` pre-fix) — `PASS`, all 6 claimed ERPNext behaviors independently reproduced verbatim, dependency-blocked delete confirmed genuinely server-side | `ACCEPTED` — both independent review passes and live QA clean; see `QA_LOG.md`'s 2026-09-24 `FIN-1E` entry for full evidence | `3c50478` (implementation) → `387590a` (fix) | None open | A background implementation agent for this package was interrupted mid-task by a session rate limit while the main session concurrently ran `git` commands in the same non-isolated working directory — produced one accidental commit scooping up unrelated foreign Sales files via a race on the shared index; caught immediately, fixed with `git reset --soft HEAD~1` (no content lost) + `git commit --only -- <path>` for every commit after; recorded as a process note for future sessions running foreground git alongside background agents sharing a working directory. `FIN-UNV-004`/`FIN-UNV-005` (GL-entries-exist branches of delete/Group↔Ledger conversion) remain genuinely unexercised — would require posting real GL Entries against a test account, deliberately not attempted to avoid polluting real ledger data | 2026-09-24 |
