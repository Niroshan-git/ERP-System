...
| MFG-CLOSE-2 — BOM Cancel/Amend | Manufacturing / Master Data (frontend) | `cancelBomAction`/`amendBomAction` on `/master-data/boms/[name]` (docstatus 1 → Cancel; docstatus 2 → Amend), new `BOM` entry in `lib/connections.ts` proactively checking Work Order as a cancel-blocker. **Provenance**: implemented by an earlier session and left fully coded but uncommitted with zero review/QA/documentation/logging — this row records the current session's cold pickup and full closure of that already-built diff, not new implementation. | `CLAUDE_HANDOFF` (closure) | `code-reviewer` (in-session) — no CRITICAL/HIGH findings, code verdict "correct and well-built"; one BLOCKING documentation-only finding (see Open Findings) | `CLAUDE_HANDOFF` — awaiting independent cross-review per `TEMP_DUAL_CLAUDE_MODE.md`; do not self-accept | Uncommitted at pickup (tip `2200cad`); this session's closure commit follows this row | Resolved same session: `bom.md` was missing the "Cancel/Amend contract" section the code's own comments cited as already existing — written from `devops`-verified source + live QA, not assumed | `devops` subagent source-verified (live v16.34.2 SSH read) that BOM cancel is blocked by two independent mechanisms — BOM's own `validate_bom_links()` (sub-assembly-only) and Frappe's generic `check_no_back_links_exist()` (all ~24 other real link-field doctypes, confirmed via live schema query) — then live-tested 3 disposable fixture scenarios (Draft-WO-doesn't-block, Submitted-WO-blocks-then-unblocks, sub-assembly-blocks-then-unblocks), all fully cleaned up and independently re-confirmed absent; the 4 real Work Orders and the 1 real BOM read-verified unchanged throughout. One factual correction to the implementation's own doc-comment: amended-BOM naming is `<cancelled-name>-<counter>` (site-wide `Document Naming Settings.default_amend_naming`), not `BOM.autoname()`'s `BOM-<ITEM>-<NNN>` scheme — no functional impact. ~20 of the ~24 confirmed link-field doctypes were schema-confirmed but not individually live-exercised (Work Order was); role/permission restrictions on cancel/amend not investigated (moot under this app's shared service-account model). Full detail in `QA_LOG.md`'s "MFG-CLOSE-2" entry and `bom.md`'s new "Cancel/Amend contract" section | 2026-09-23 |
