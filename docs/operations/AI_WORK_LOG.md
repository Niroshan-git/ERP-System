...
| `FIN-1` — Chart of Accounts (read) + Bank Account CRUD | Finance (frontend, first package) | New Finance module: `/accounting` (home), `/accounting/chart-of-accounts` (read-only `Account` tree, company-selectable, new `ChartOfAccountsTree.tsx` — server-rendered `<details>/<summary>`, no client JS), `/accounting/bank-accounts` (list/new/`[name]` detail-edit-delete, new `BankAccountForm.tsx`), new `lib/financeDefaults.ts`, new `maskSensitive()` in `lib/format.ts`. Sidebar gained a "Finance" module (`/accounting` prefix, matching the pre-existing dead `/accounting/payment-entries` Connections-tab link left for FIN-2). No `lib/erpnext.ts` changes; `deleteDoc` used for the first time anywhere in this app. Built on `FIN-0` (discovery) + `FIN-GOV-1` (governance authorization), both closed earlier the same day. | `CLAUDE_HANDOFF` | Not yet reviewed | `CLAUDE_HANDOFF` — awaiting independent review per `TEMP_DUAL_CLAUDE_MODE.md` while still in its effective window (through 2026-09-25); not self-accepted | Not yet committed as of this row — see `PROGRESS.md`/later commits | None yet — in-session `npm run lint`/`npm run build` both clean for every file this package touched (one pre-existing, unrelated `no-explicit-any` error in `sales/delivery-notes/actions.ts`, foreign/untouched code) | `FIN-UNV-001` (Bank Account delete-blocked-by-link exact message — no linked record exists yet, Payment Entry is FIN-2 scope), `FIN-UNV-002` (`validate_account()` duplicate-Account-link wording — not exercised), `FIN-UNV-003` (base, non-`System-Manager`-override DocPerm grant on Bank Account for `Accounts Manager`/`Accounts User`). Live-verified (not source-derived): 96-account tree with zero orphan `parent_account` references; a real test Bank (`CS-TEST-BANK-FIN1`) and Bank Account (`CS-TEST-FIN1-ACCOUNT - CS-TEST-BANK-FIN1`) created via the exact REST calls the server actions make, read back, updated, duplicate-name-rejected (409), then deleted and confirmed gone — zero leftover test data. Native IBAN format validation (`frappe.utils.validate_iban`) and `is_company_account`'s conditional Company/Account requirement both live-confirmed. No authenticated browser session available (consistent with every prior package this repo has shipped) — UI was verified via a clean production build plus REST-level behavioral equivalence testing, not an interactive click-path. | 2026-09-24 |
...
| `SALES-CANCEL-1` — Sales Connection Map & Cancellation Blockers | Sales (frontend) | Updated `lib/connections.ts` with missing downstream connections (Material Request, Purchase Order, Payment Entry, Stock Entry). Refactored cancellation actions across Sales Orders, Delivery Notes, Quotations, and Sales Invoices to use a generic pattern that iterates over all downstream connections rather than hardcoded checks. | `CLAUDE_HANDOFF` | Not yet reviewed | `CLAUDE_HANDOFF` — pending Codex's independent review | uncommitted — pending this package's commit | None open | Full authenticated browser click-path not run | 2026-09-24 |
