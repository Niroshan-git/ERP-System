...
import { SIMPLE_REPORTS } from "@/lib/reports";
import { ReportFilterBar, type ReportFilterFieldConfig } from "@/components/ReportFilterBar";
import { ReportTable } from "@/components/ReportTable";

/**
 * One generic page for all 14 "simple" reports in SIMPLE_REPORTS (see that file's doc
 * comment for why they share a page instead of 14 near-duplicate files). Resolves each
 * field's ERPNext filter value (Link fields fetch their options live; dates resolve
 * "today"/"year-start"; everything else falls back to its configured default), then
 * runs the report exactly like the three hand-built report pages do.
 */
...
export default async function SimpleReportPage({
  ...
  const config = SIMPLE_REPORTS.find((r) => r.slug === slug);
  if (!config) notFound();
