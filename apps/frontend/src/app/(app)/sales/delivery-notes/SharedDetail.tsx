...
export async function SharedDetail({
  name,
  saved,
  expectedReturn,
}: {
    ...
    if (e instanceof ErpNextError && e.status === 403) {
      return (
        <div>
          ...
          <AccessDeniedNotice what={expectedReturn ? "this sales return" : "this delivery note"} />
        </div>
      );
    }
    throw e;
  ...
  if (expectedReturn && doc.is_return !== 1) {
  ...
  const breadcrumb = (
    <Breadcrumb
      items={[
        { label: "Home", href: "/" },
        { label: "Selling", href: expectedReturn ? "/sales/returns" : "/sales/delivery-notes" },
        { label: expectedReturn ? "Sales Return" : "Delivery Note", href: expectedReturn ? "/sales/returns" : "/sales/delivery-notes" },
        { label: doc.customer },
      ]}
    />
  );
  ...
  if (doc.docstatus === 0 && doc.is_return !== 1) {
    ...
    detailsTab = (
      <div>
        ...
        <dl className="mb-4 grid grid-cols-2 gap-4 text-sm md:grid-cols-4">
          {expectedReturn && doc.return_against && (
            <div className="col-span-2 md:col-span-4 mb-2">
              <span className="text-graphite-500 mr-2">Original Delivery Note:</span>
              <a href={`/sales/delivery-notes/${encodeURIComponent(doc.return_against)}`} className="font-mono text-signal hover:underline">
                {doc.return_against}
              </a>
            </div>
          )}
          ...
          <DocField label="Grand total" value={`${formatAmount(expectedReturn ? Math.abs(doc.grand_total) : doc.grand_total)} ${doc.currency}`} mono />
        </dl>
        <LineItemsTable items={doc.items} currency={doc.currency} isReturn={expectedReturn} />
      </div>
    );
