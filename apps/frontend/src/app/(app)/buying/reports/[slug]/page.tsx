...
import { BUYING_SIMPLE_REPORTS } from "@/lib/buyingReports";
import { ReportFilterBar, type ReportFilterFieldConfig } from "@/components/ReportFilterBar";
import { ReportTable } from "@/components/ReportTable";

/**
 * One generic page for every "simple" Buying report in BUYING_SIMPLE_REPORTS — the Buying
 * analog of `/reports/[slug]/page.tsx` (see that file and lib/buyingReports.ts's doc
 * comment for why they share a page instead of one near-duplicate file per report).
 */

type SearchParams = Record<string, string | undefined>;

export default async function BuyingSimpleReportPage({
  params,
  searchParams,
}: {
  ...
  const config = BUYING_SIMPLE_REPORTS.find((r) => r.slug === slug);
  if (!config) notFound();
